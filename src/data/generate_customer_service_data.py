"""Generate synthetic preference data for the customer-service reward model.

This module sits at the very start of the project pipeline:
1. it calls a general-purpose LLM through :mod:`src.model.llm`,
2. converts each generated sample into the project's canonical
   ``context_messages + winner`` schema,
3. writes JSONL shards under ``data/raw/`` for later split/preprocess/training.

Input:
- a model name exposed by the OpenAI-compatible wrapper in ``src.model.llm``
- prompt templates, scenario categories, and generation count

Output:
- JSONL records with ``context_messages`` and ``winner``
- files that are later consumed by ``src.data.split`` and then by
  ``src.model.prompt_template`` / ``src.training``
"""
...
from src.model.llm import call_llm
...
def generate_one_data_item(model_name: str = "gpt-5") -> Dict:
    """Generate one raw preference sample and normalize it to project format.

    The returned record is already aligned with downstream training code:
    ``context_messages`` becomes the prompt field consumed by
    ``src.data.rl_dataset.RubricRMDataset``, while ``winner`` becomes the
    ground-truth label used by the reward function.
    """
...
def generate_dataset(
    ...
    output_file: str = "data/raw/customer_service_dataset.jsonl",
    model_name: str = "gpt-5",
    num_workers: int = 6
):
    """Generate a full raw dataset with multi-threaded LLM calls.

    Args:
        total_count: Number of generation attempts. Some may fail and be skipped.
        save_interval: How often to checkpoint the current successful samples.
        output_file: JSONL destination under ``data/raw/``.
        model_name: Upstream model name passed to ``call_llm``.
        num_workers: Thread count used to parallelize API calls.

    Output:
        A JSONL file that can be merged and split by ``src.data.split``.
    """
    dataset = []
...
def preview_data(output_file: str = "customer_service_dataset.jsonl", num_samples: int = 3):
    """Print a few generated samples for manual quality inspection."""
...
if __name__ == "__main__":
    default_output = Path("data/raw/customer_service_dataset.jsonl")
    ...
    # 预览数据
    preview_data(str(default_output), num_samples=3)
