"""Build mixed preference datasets from public sources.

This module is mainly for research-time dataset construction rather than the
customer-service-only pipeline. It converts several public preference datasets
into the same ``context_messages + winner`` format used everywhere else in the
repository, so the resulting JSONL files can be consumed by
``src.data.rl_dataset`` and the GRPO training stack without extra adapters.
"""
...
def convert_sky_data_single(input):
    """Convert a single-turn Skywork sample into ``question, answer`` strings."""
...
def convert_sky_data_multi(input, assistant_name="Assistant"):
    """Flatten a multi-turn chat into a readable transcript string."""
    result_parts = []
    for entry in input:
        role = entry['role']
...
def get_smaller_dataset_sky():
    """Build a filtered Skywork subset in the project's pairwise preference format."""
    global CURRENT_NUM
...
def collect_code_data_sky():
    """Convert code preference pairs into the shared pairwise schema."""
    global CURRENT_NUM 
