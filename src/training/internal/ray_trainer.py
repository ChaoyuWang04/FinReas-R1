"""Project-specific extension of verl's ``RayPPOTrainer``.

This file deliberately changes only one layer of the upstream trainer: data
loading. The rest of the PPO / GRPO optimization logic stays inside verl. That
design keeps the repository focused on project-owned concerns:
- how our JSONL data is read,
- how train/val loaders are built,
- how total training steps are injected back into optimizer schedules.
"""
...
from src.data.rl_dataset import RubricRMDataset
...
class RubricRMRayPPOTrainer(_RayPPOTrainer):
    ...
    def _create_dataloader(self):
        """
        Build train/validation loaders around the repository's JSONL schema.

        This is the key method that connects:
        - ``src.data.rl_dataset`` on the data side
        - verl's rollout / optimization engine on the training side
        """
        
        # --- 1. 创建训练数据集 ---
        # TODO: the effective batch size should still be checked against data parallel degree.
        ...
        self.train_dataloader = StatefulDataLoader(
            ...
            # ``train_batch_size`` 是 rollout 阶段聚合后的全局 batch 大小，通常先从
            # 脚本里控制，再由内部 worker 根据 world size 做进一步归一化。
