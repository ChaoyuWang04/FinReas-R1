"""Customized FSDP workers used by the GRPO training stack.

This module is the lowest-level project-owned training code in the repository.
It adapts verl's worker concept to the specific needs of this project:
- actor / rollout / reference-policy worker composition,
- FSDP and sequence-parallel normalization,
- model / optimizer construction for local reward-model experiments.

Most business logic still lives in verl itself. This file mainly exists because
training-scale details such as prompt lengths, batch normalization across world
size, and local model loading need project-specific control.
"""
...
class ActorRolloutRefWorker(Worker):
    """
    Unified FSDP worker used for actor, rollout, and reference-policy roles.

    Depending on ``role``, the same class can:
    - host the trainable policy (actor),
    - run generation / logprob computation (rollout),
    - host the frozen KL reference policy (ref).
    """

    def __init__(self, config: DictConfig, role: str):
        """Initialize distributed state and normalize batch-size-related hyperparameters.

        A large portion of the constructor is about translating human-facing
        config values into per-worker values. For example, ``ppo_mini_batch_size``
        and micro-batch settings are divided by world size or sequence-parallel
        degree so the optimizer sees the intended effective batch.
        """
        super().__init__()
        self.config = config
        import torch.distributed
        ...
        # normalize config
        # 这些超参在 YAML / shell 脚本里通常按“全局训练规模”设置；这里把它们转成
        # 单 worker 实际执行时使用的数值。
        if self._is_actor:
        ...
        # ``log_prob_micro_batch_size`` 一般在显存不够时优先调小；默认值常常与
        # ``ppo_micro_batch_size_per_gpu`` 同量级。
    ...
    def _build_model_optimizer(self,
        ...
        """Build the transformer module plus optional optimizer/scheduler.

        Important training hyperparameters handled here include:
        - ``model_dtype`` / mixed precision: controls memory footprint and stability
        - ``enable_gradient_checkpointing``: trades compute for memory, commonly on
        - optimizer ``lr`` and warmup settings: control adaptation speed
        - FSDP wrap / offload settings: control memory sharding strategy
        """
        ...
        if torch_dtype is None:
            # 常见默认：actor 用 fp32 初始化更安全，ref 可以更激进地直接用 bf16。
        ...
        # mixed precision 通常默认采用 bf16 参数 + fp32 归约，兼顾显存与稳定性。
        ...
        if role == 'actor' and optim_config is not None:
            ...
            # ``lr_warmup_steps`` / ``lr_warmup_steps_ratio`` 是训练里很常见的两个控件：
            # - constant warmup: 适合先稳定再平稳训练
            # - cosine warmup: 适合更长训练周期
